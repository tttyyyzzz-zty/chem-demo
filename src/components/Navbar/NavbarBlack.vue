    ...
    <div class="gpt3__navbar-links">
      ...
      <div class="gpt3__navbar-links_container">
        <p v-for="({ name, href }, index) in menuItems" :key="index + name">
          <a :href="href" style="color: black;font-weight: bold;">{{ name }}</a>
        </p>
      </div>
    </div>
    <div class="gpt3__navbar-sign">
      <el-dropdown>
        <span style="font-weight: bold;" @click="oppsignin">
                    {{user.username}}
      ...
      <button type="button" @click="handleToggleMenu" style="color: black;font-weight: bold;">Sign up</button>
    </div>
...
<script>
import logo from "../../assets/ai_lab.svg";
